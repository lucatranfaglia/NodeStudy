...
// Ottengo i todo tramite una lista (listId) - parametro 'listId' , middleware (logger), middleware
router.get('/:listId([0-9]+)/todos', logger, async (req, res)=>{
    try {
...
// Ottengo l'utente tramite ListID
router.get('/:userId([0-9]+)/user', async (req, res)=>{
    try {
        const userId = req.params.userId;
        const result = await getUserByList(userId);
        res.status(result ? 200 : 404).json(result ? result : "User not found!");
    } 
    catch (error) {
        res.status(500).send(error.toString());
    }
